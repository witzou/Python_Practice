...
def voc_eval(detpath,
    ...
    npos = 0  # 记录一共含有的object数目，也就是所有图片的box总数
    ...
    # read dets 读取某类别预测输出
    detfile = detpath.format(classname) # clw note：如./AB.txt 也就是记录所有预测数据的那个txt
    ...
    for d in range(nd):
        ...
        # clw note：如果预测输出的是(x_min, y_min, x_max, y_max)，要把下面屏蔽掉，因为无需转换
...
class_name = 'AB'
rec, prec, ap = voc_eval('./{}.txt', './Annotations/{}.xml', './test.txt', class_name, './')
print('clw:rec = ', rec)
print('clw:prec = ', prec)
print('clw:ap = ', ap)
print('\n')
