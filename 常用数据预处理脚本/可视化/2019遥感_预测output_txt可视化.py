...
def draw_boxes(image_path):
    ...
    for data in txt_datas:
        ...
        y1 = float(data.split(' ')[3])
        x2 = float(data.split(' ')[4])
        y2 = float(data.split(' ')[5])
        x3 = float(data.split(' ')[6])
        y3 = float(data.split(' ')[7])
        x4 = float(data.split(' ')[8])
        y4 = float(data.split(' ')[9])
...
def main():
    data_path = '/media/clwclw/data/DOTA_15classes/val_test'
    print(data_path)
