...
img_path = '/media/clwclw/Elements/deep_learning/competion/2019yaogan/train/train_crop_800_bigger_64/images/'
annFile = '/media/clwclw/Elements/deep_learning/competion/2019yaogan/train/train_crop_800_bigger_64/train.json'
...
def draw_rectangle(coordinates, image, image_name):
    for coordinate in coordinates:
        ...
        top = np.rint(coordinate[1])
        right = np.rint(coordinate[2])
...
for i in range(7):
    ...
    for j in range(len(anns)):
        coordinate = []
        coordinate.append(anns[j]['bbox'][0])  # 这里默认COCO的bbox表示方法是xywh，当然如果实际是x1y1x2y2形式，改一下就行了
