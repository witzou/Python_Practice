...
'''
#################################################
clw note:这里需要根据实际情况修改成其他的txt文件
#################################################
'''

# input_path = 'UCSD_train.txt'
input_path = '/media/clwclw/data/2018yuncong/Part_B_train.txt'
# input_path = 'Mall_train.txt'

img_path = '/media/clwclw/data/2018yuncong'
...
with open(input_path, 'r') as f:
    ...
    for line in lines:
        ...
        # clw note：image_annotation_list[0]就是文件名，如train/ourmall/image_7613.jpg
        imageName = image_annotation_list[0].split('/')[-1]  # clw note：这里是带.jpg的
        imagePath = imageName

        ##clw note：相当于D:/Deep-Learning/dataset/yuncong/yuncong_data/UCSD/train_data/vidf1_33_000_f170.jpg
        img = Image.open(os.path.join(img_path, image_annotation_list[0]))

        # clw modify 20190102：Mall数据集的txt有1500条记录，但是Mall有2000张图片，因此
        #                   考虑首先根据txt的记录，筛选出这1500张图片，并保存到另一个文件夹
        # img.save('D:/Deep-Learning/dataset/yuncong/yuncong_data/Mall_new/' + image_annotation_list[0].split('/')[-1])
        ...
        save_folder = img_path + '/annotations'
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        outputFile = save_folder + '/' + imageName.split('.')[0] + '.xml'
        writeToXml(imageName, imageSize, imagePath, allCellInfo, outputFile)
        print('clw:imageName=',imageName, "written to", outputFile)
        count += 1
        print('clw: xml count = ', count)
